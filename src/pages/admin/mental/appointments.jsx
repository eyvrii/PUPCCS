...
export default function MentalAppointments() {
  ...
  const fetchAppointments = async () => {
    const { data } = await supabase
      .from('appointments')
      .select('*')
      .in('concern_type', CLINIC_CONCERNS.mental)
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Total Mental Health Appointments</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-5">
          <div className={`bg-white rounded-xl border border-gray-100 shadow-sm overflow-x-auto ${selected ? 'flex-1' : 'w-full'}`}>
            {loading ? (
              ...
              <p className="text-sm text-gray-400 p-6">No mental health appointments found.</p>
            ) : (
